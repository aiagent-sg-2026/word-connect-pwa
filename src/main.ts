...
import { hintPlan } from './game/logic';
...
function openHintSheet() { const options: [HintKind,string,string,string][] = [['letter','Reveal a letter',`${HINT_COSTS.letter} coins`,'Reveals the next hidden letter'],['first-letter','First letter',`${HINT_COSTS['first-letter']} coins`,'Starts an untouched answer with its first letter'],['word','Reveal word',`${HINT_COSTS.word} coins`,'Shows one unsolved answer']]; sheet('Choose a hint', `<p class="sheet-note">Hints reveal progress in this puzzle and persist in your save.</p><div class="sheet-options">${options.map(([kind,label,cost,desc]) => { const useful = !!hintPlan(level, progress, kind); const affordable = profile.coins >= HINT_COSTS[kind]; const state = !useful ? 'No new information' : !affordable ? `Need ${HINT_COSTS[kind] - profile.coins} more` : 'Available now'; return `<button class="hint-choice" data-hint-kind="${kind}" ${!useful || !affordable ? 'disabled' : ''}><strong>${label}<span>${cost}</span></strong><small>${desc} · ${state}</small></button>`; }).join('')}</div>`); document.querySelectorAll<HTMLButtonElement>('[data-hint-kind]').forEach(b => b.addEventListener('click', async () => { const kind = b.dataset.hintKind as HintKind; const res = await useHint(level, kind); closeSheet(); profile = res.profile; progress = res.progress; if (res.charged) { play('hint'); markFeedback('hint'); } render(res.charged ? (kind === 'word' ? 'Word revealed' : 'Letter revealed') : res.hint ? `Need ${HINT_COSTS[kind]} coins` : 'No unsolved hint available'); })); }
