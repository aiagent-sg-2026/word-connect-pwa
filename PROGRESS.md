...
- Added `score-v1-provisional` with versioned weights/thresholds, short-word strictness, per-word explanations, and starter Golden Set evaluation. Added Human Golden Set Workflow V1 as build-time tooling with versioned contracts, deterministic bounded queue, blind review JSON/CSV export, strict review validation, two-independent-review/disagreement/adjudication model, draft/production publication gates, deterministic protected holdout split, status coverage, and frozen evaluator reporting. Current queue is 187 candidates with shortfall 1,813; human reviewed/resolved count is 0. The ~2,000 human-reviewed Golden Set remains pending and Phase 5 is not production-complete.
...
- Phase 4/5/6 remain LAB/provisional rather than production-complete: no licensed production corpus, no ~2,000-word human Golden Set, Golden queue is DRAFT (187/2,000; 0 resolved human words), and QA seed corpus currently generates 94 unique LAB levels for the default seed/scale.
