...
- Phase 5 reviewer surface V1 is implemented separately under `/word-connect-pwa/reviewer/`; operational verification is covered by `npm run reviewer:verify` and `npm run reviewer:smoke`. Phase 5 itself remains incomplete until two real independent reviews per production word and adjudication of disagreements are imported.
