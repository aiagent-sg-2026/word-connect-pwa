...
npm run golden:review:export     # blind JSON + CSV packet; no predicted class/scores
npm run golden:review:validate   # validate data/golden/reviews/human-reviews-v1.json
...
A valid human review must include an opaque `reviewerId`, ISO `reviewedAt`, `source: "human-review-v1"`, class, confidence `0..1`, and optional reason/note. Model-generated or policy-generated labels cannot satisfy the human gate. Validation rejects unknown words, duplicate same-reviewer/word rows, invalid class/source/timestamp/confidence, and queue version/checksum mismatches.

A production final item requires two independent agreeing human reviews, or unresolved disagreement plus explicit human adjudication (`source: "human-adjudication-v1"`) recording adjudicator, timestamp, final class, source reviewer IDs, and optional note. Policy/model overrides cannot auto-resolve human disagreement.

`npm run golden:publish` refuses READY unless: candidate count is at least 2,000, all items are resolved, no duplicates exist, queue checksum/version is frozen, and split integrity passes. `--draft` is deliberately separate and never means 2,000 human-reviewed words exist.

## Frozen evaluator principle

For a comparison run, freeze evaluator version, thresholds/scoring version, source dictionary checksum, candidate/final Golden checksum, split policy, and baseline scoring version. Missing or unverifiable human evidence is `UNKNOWN/PENDING`, never PASS. Evaluator changes require a new evaluator version and new baseline; expected labels must not be tuned to make the current scorer pass.
