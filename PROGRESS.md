...
- Added `score-v1-provisional` with versioned weights/thresholds, short-word strictness, per-word explanations, and starter Golden Set evaluation. Added Human Golden Set Workflow V1 as build-time tooling with versioned contracts, deterministic bounded queue, blind review JSON/CSV export/import, strict review/adjudication validation, two-distinct-human review invariants, third-party adjudication only for real disagreements, fixed 2,000 production floor independent of queue target, fail-closed queue/published-artifact checksum verification, deterministic protected holdout split, status/publish readiness parity, and frozen evaluator reporting. Current queue is 187 candidates with shortfall 1,813; human reviewed/resolved count is 0. The ~2,000 human-reviewed Golden Set remains pending and Phase 5 is not production-complete.
...
- Added the separate Phase 5 Human Golden Reviewer PWA (`reviewer-dist/`): deterministic production blind packet preparation, isolated IndexedDB settings/reviews/history, explicit reviewer ID, one current row per reviewer/word, strict queue-identity import/export compatible with `parseReviewsFile`/`validateReviews`, fail-closed multi-reviewer combine tooling, keyboard/touch navigation, skip/defer, privacy notice, and dedicated offline shell. No production human reviews are fabricated; the queue remains DRAFT pending real collection and adjudication.
