...
import { verifyEsdbIntegrity } from '../src/content-pipeline/integrity.ts';
...
const integrityManifestPath = 'content/corpus/esdb-en-us-v1.integrity.json';
...
const pinnedCommitAvailable = spawnSync('git', ['-C', cache, 'cat-file', '-e', `${rev}^{commit}`]).status === 0;
if (!pinnedCommitAvailable) run('git', ['-C', cache, 'fetch', '--tags', 'origin', rev]);
...
verifyEsdbIntegrity({ manifest: integrityManifest, actualRevision: actual, dbPath, licensePath: `${cache}/Copyright` });
...
verifyEsdbIntegrity({ manifest: integrityManifest, actualRevision: actual, dbPath, rows, licensePath: `${cache}/Copyright` });
