...
async function submitSelected() {
  ...
  try { const word = tileWord(selected, level.letters); const res = await submitWord(level, word); const firstComplete = !progress.completed && res.progress.completed; profile = res.profile; progress = res.progress; stats = await getStats(); selected = []; if (res.unlocked.length) { achievementToast = `Achievement unlocked: ${ACHIEVEMENTS.find(a => a.id === res.unlocked[0])?.title}`; if (achievementToastTimer) clearTimeout(achievementToastTimer); achievementToastTimer = setTimeout(() => { achievementToast = ''; render(); }, 2400); } const cue: FeedbackCue = firstComplete ? 'complete' : res.outcome.kind === 'TARGET' ? 'target' : res.outcome.kind === 'BONUS' ? 'bonus' : res.outcome.kind === 'ALREADY_FOUND' ? 'already-found' : 'invalid'; play(cue); markFeedback(firstComplete ? 'complete' : cue); coinsChanged = !!res.outcome.coinsDelta || firstComplete; render(`${res.outcome.message}${(profile.combo ?? 0) >= 2 ? ` · Combo ×${profile.combo}` : ''}`); } catch { selected = []; play('invalid'); markFeedback('invalid'); render('Tile cannot be reused'); }
}
