...
export async function useHint(level: LevelContract): Promise<{hint?: string; profile: ProfileRecord; progress: ProgressRecord}> {
  const db = await openGameDb();
  ...
  if (hint && profile.coins >= 10) { profile.coins -= 10; profile.hintsUsed++; profile.updatedAt = now(); await tx.objectStore('profiles').put(profile); await tx.objectStore('progress').put(progress); await tx.objectStore('economyEvents').add({ profileId: PROFILE_ID, levelId: level.levelId, kind: 'HINT', coinsDelta: -10, hint, createdAt: now() }); }
  await tx.done;
  return { hint, profile, progress };
}
...
export async function importSave(envelope: any): Promise<void> { if (!envelope || envelope.envelope !== 'word-connect-save-v1' || envelope.campaignHash !== CAMPAIGN.campaignHash || !envelope.data?.profiles) throw new Error('REC_IMPORT_INVALID'); const snapshot = await exportSave(); const db = await openGameDb(); const tx = db.transaction(['profiles','progress','settings','economyEvents','statsDaily','saveSnapshots'], 'readwrite'); await tx.objectStore('saveSnapshots').add({ createdAt: now(), reason: 'pre-import', data: snapshot }); for (const store of ['profiles','progress','settings','economyEvents','statsDaily']) for (const item of envelope.data[store] || []) await tx.objectStore(store).put(item); await tx.done; }
