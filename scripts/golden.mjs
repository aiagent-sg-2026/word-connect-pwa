...
function combineInputPaths() { const args = process.argv.slice(3); const files = []; for (let i=0;i<args.length;i++) { if (args[i] === '--production') continue; if (args[i] === '--out') { i++; continue; } files.push(args[i]); } return files; }
if (!cmd) { console.error('Usage: golden <queue|review:export|review:validate [--file PATH]|review:import --file PATH|review:combine --out PATH FILE...|adjudication:validate [--file PATH]|status|publish|eval>'); process.exit(2); }
...
else if (cmd === 'review:combine') {
  const q = existsSync(queuePath) ? loadQueue() : await queue(); const out = argValue('--out'); const files = combineInputPaths();
  if (!out || files.length < 1) throw new Error('review:combine requires --out PATH and at least one review JSON/CSV file');
  const reviews = files.flatMap(path => { const rows = parseReviewsFile(path); if (path.toLowerCase().endsWith('.json')) { const raw = JSON.parse(readFileSync(path,'utf8')); if (raw && !Array.isArray(raw) && typeof raw.reviewerId === 'string' && rows.some(r => r.reviewerId !== raw.reviewerId)) throw new Error(`review:combine wrapper reviewerId mismatch in ${path}`); } return rows; });
  const errors = validateReviews(q, reviews); if (errors.length) { console.error(JSON.stringify({ combined:false, files, errors }, null, 2)); process.exit(1); }
  const reviewerIds = [...new Set(reviews.map(r => r.reviewerId))].sort(); writeJson(out, { schemaVersion:'human-golden-reviews-v1', queueVersion:q.source.queueVersion, queueChecksum:q.checksum, reviewerIds, reviews });
  console.log(JSON.stringify({ combined:true, out, files:files.length, reviewerIds, reviewCount:reviews.length }));
}
